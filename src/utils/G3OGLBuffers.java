...
public class G3OGLBuffers {
	...
	public G3OGLBuffers(GL3 gl) {
		this.gl = gl;
	}

	public G3OGLBuffers(GL3 gl, float[] vertexData, Attrib[] attributes, int[] indexData) {
		this.gl = gl;
		addVertexBuffer(vertexData, attributes);
		if (indexData != null) setIndexBuffer(indexData);
	}

	public G3OGLBuffers(GL3 gl, float[] vertexData, int floatsPerVertex, Attrib[] attributes, int[] indexData) {
		this.gl = gl;
		addVertexBuffer(vertexData, attributes);
		if (indexData != null) setIndexBuffer(indexData);
	}
	...
	public void addVertexBuffer(float[] data, int floatsPerVertex, Attrib[] attributes) {
		...
		gl.glBindBuffer(GL3.GL_ARRAY_BUFFER, bufferID[0]);
		gl.glBufferData(GL3.GL_ARRAY_BUFFER, data.length * 4, buffer,
				GL3.GL_STATIC_DRAW);
	...
	public void setIndexBuffer(int[] data) {
		...
		gl.glBindBuffer(GL3.GL_ELEMENT_ARRAY_BUFFER, indexBuffer[0]);
		gl.glBufferData(GL3.GL_ELEMENT_ARRAY_BUFFER, data.length * 4, buffer,
				GL3.GL_STATIC_DRAW);
	}
	...
	public void bind(int shaderProgram) {
		...
		for (VertexBuffer vb : vertexBuffers) {
			gl.glBindBuffer(GL3.GL_ARRAY_BUFFER, vb.id);
			int offset = 0;
			for (int j = 0; j < vb.attributes.length; j++) {
				...
				gl.glVertexAttribPointer(location, vb.attributes[j].dimension,
						GL3.GL_FLOAT, vb.attributes[j].normalize, vb.stride,
	...
	public void draw(int topology, int shaderProgram) {
		...
		if (indexBuffer == null) {
			...
			gl.glDrawElements(topology, indexCount, GL3.GL_UNSIGNED_INT, 0);
		}
		unbind();
	}
	...
	public void draw(int topology, int shaderProgram, int count) {
		...
		if (indexBuffer == null) {
			...
			gl.glDrawElements(topology, count, GL3.GL_UNSIGNED_INT, 0);
		}
		unbind();
	}
	...
	public void draw(int topology, int shaderProgram, int count, int start) {
		...
		if (indexBuffer == null) {
			...
			gl.glDrawElements(topology, count, GL3.GL_UNSIGNED_INT, start);
		}
		unbind();
	}
